...
void __attribute__ ((constructor)) CAN_init(void){
    ...
    mcp2515_bit_modify(MCP_RXB0CTRL,    MCP_RXB0CTRL__SET_FILTERS_OFF);
    mcp2515_bit_modify(MCP_RXB0CTRL,    MCP_RXB0CTRL__SET_ROLLOVER_OFF);

    mcp2515_bit_modify(MCP_CANINTE,     MCP_CANINTE__SET_RX0_FULL_ENABLE);
    
    mcp2515_bit_modify(MCP_CANCTRL,     MCP_CANCTRL__SET_MODE_NORMAL);
}

void CAN_send(can_msg_t msg){
    while((mcp2515_read(MCP_TXB0CTRL) & MCP_TXB0CTRL__TEST_TX_REQUEST)){}
...
can_msg_t CAN_recv(void){
    ...
    if(mcp2515_read(MCP_CANINTF) & MCP_CANINTF__TEST_RX0_FULL){
        ...
        mcp2515_bit_modify(MCP_CANINTF, MCP_CANINTF__SET_RX0_CLEAR);
