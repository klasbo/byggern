// Copyright (c) 2014 Tero Hanninen
// Boost Software License - Version 1.0 - August 17th, 2003
module imageformats_32bit_edit;    // version 2.0.0
...
ubyte[] read_IDAT_stream(ref PNG_Decoder dc, int len) {
    ...
    ubyte[] depaletted_line = dc.src_indexed ? new ubyte[to!uint(dc.w * 3)] : null;
    ubyte[] result = new ubyte[to!uint(dc.w * dc.h * dc.tgt_chans)];
    ...
    void depalette_convert(in ubyte[] src_line, ubyte[] tgt_line) {
        for (long s, d;  s < src_line.length;  s+=1, d+=3) {
            long pidx = src_line[to!uint(s)] * 3;
            if (dc.palette.length < pidx + 3)
                throw new ImageIOException("palette idx wrong");
            depaletted_line[to!uint(d) .. to!uint(d+3)] = dc.palette[to!uint(pidx) .. to!uint(pidx+3)];
        }
    ...
    if (dc.ilace == InterlaceMethod.None) {
        ...
        auto cline = new ubyte[to!uint(src_sl_size+1)];   // current line + filter byte
        auto pline = new ubyte[to!uint(src_sl_size+1)];   // previous line, inited to 0
        ...
        foreach (j; 0 .. dc.h) {
            ...
            convert(cline[1 .. $], result[to!uint(tgt_si) .. to!uint(tgt_si + tgt_sl_size)]);
            tgt_si += tgt_sl_size;
        ...
        const linebuf0 = new ubyte[to!uint(max_scanline_size+1)]; // +1 for filter type byte
        const linebuf1 = new ubyte[to!uint(max_scanline_size+1)]; // +1 for filter type byte
        auto redlinebuf = new ubyte[to!uint(dc.w * dc.tgt_chans)];

        foreach (pass; 0 .. 7) {
            const A7_Catapult tgt_px = a7_catapults[pass];   // target pixel
            const long src_sl_size = redw[pass] * filter_step;
            auto cline = cast(ubyte[]) linebuf0[0 .. to!uint(src_sl_size+1)];
            auto pline = cast(ubyte[]) linebuf1[0 .. to!uint(src_sl_size+1)];
            ...
            foreach (j; 0 .. redh[pass]) {
                ...
                recon(cline[1..$], pline[1..$], filter_type, filter_step);
                convert(cline[1 .. $], redlinebuf[0 .. to!uint(redw[pass]*dc.tgt_chans)]);

                for (int i, redi; i < redw[pass]; ++i, redi += dc.tgt_chans) {
                    long tgt = tgt_px(i, j, dc.w) * dc.tgt_chans;
                    result[to!uint(tgt) .. to!uint(tgt + dc.tgt_chans)] =
                        redlinebuf[redi .. to!uint(redi + dc.tgt_chans)];
                }
...
void write_IDATs(ref PNG_Encoder ec) {
    ...
    ubyte[] cline = new ubyte[to!uint(linesize)];
    ubyte[] pline = new ubyte[to!uint(linesize)];
    debug(DebugPNG) assert(pline[0] == 0);

    ubyte[] filtered_line = new ubyte[to!uint(linesize)];
    ...
    long si = 0;
    foreach (j; 0 .. ec.h) {
        convert(ec.data[to!uint(si) .. to!uint(si+src_line_size)], cline[1..$]);
        si += src_line_size;
...
void write_to_IDAT_stream(ref PNG_Encoder ec, in void[] _compressed) {
    ...
    while (compressed.length) {
        ...
        ec.data_buf[ec.writelen .. to!uint(ec.writelen + writenow_len)] =
            compressed[0 .. to!uint(writenow_len)];
        ec.writelen += writenow_len;
        compressed = compressed[to!uint(writenow_len) .. $];
...
class Reader {
    ...
    void mem_readExact(ubyte[] buffer, size_t bytes) {
        if (source.length - cursor < bytes)
            throw new Exception("not enough data");
        buffer[0..bytes] = source[to!uint(cursor) .. to!uint(cursor+bytes)];
        cursor += bytes;
    }
        ...
        switch (origin) {
            ...
            case SEEK_END:
                if (0 <= offset || source.length < -offset)
                    throw new Exception("seek error");
                cursor = cast(int) source.length + offset;
                break;
            default: assert(0);
        }
    }
